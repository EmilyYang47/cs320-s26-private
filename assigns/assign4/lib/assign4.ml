...
let check_rule (rname : ty_rule) (premises : ty_jmt option list) (concl : ty_jmt) : bool =
  ...
  | Mul_int -> (match premises with
                | [e1; e2] -> (match concl with
                                        | {expr = Bop (Mul, c1, c2); ty = ty} -> (match e1 with
                                                                                | None -> true
                                                                                | Some e ->
                                                                                    e.expr = c1 &&
                                                                                    e.ty = IntT
  ...
  | Eq_rule -> (match premises with
                | [e1; e2] -> (match concl with
                                        | {expr = Bop (Eq, c1, c2); ty = ty} -> (match (e1, e2) with
                                                                                | (Some pe1, Some pe2) ->
                                                                                    pe1.expr = c1 &&
                                                                                    pe2.expr = c2 &&
                                                                                    pe1.ty = pe2.ty && 
  ...
  | If_rule -> (match premises with
                | [e1; e2; e3] -> (match concl with
                                        | {expr = If (c1, c2, c3); ty = ty} -> (match e1 with
                                                                                | Some e -> e.expr = c1 && e.ty = BoolT 
                                                                                | _ -> true
                                                                                ) 
                                                                                && 
                                                                                (match (e2, e3) with
...
let example_deriv : string = "((if (= (= 5 (+ 1 4)) (= 0 1)) (+ 2 3) (* (+ 4 5) 67)) int IF
(   (= (= 5 (+ 1 4)) (= 0 1)) bool EQ    (  (= 5 (+ 1 4)) bool EQ    (5 int INTLIT)    ((+ 1 4) int ADDINT    (1 int INTLIT)    (4 int INTLIT)) )    ((= 0 1) bool EQ   (0 int INTLIT)   (1 int INTLIT))   )

((+ 2 3) int ADDINT   (2 int INTLIT)   (3 int INTLIT))

((* (+ 4 5) 67) int MULINT    ((+ 4 5) int ADDINT (4 int INTLIT)  (5 int INTLIT))   (67 int INTLIT)) 
